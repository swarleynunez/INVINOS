/*
 * API for INVINOS blockchain broker
 *
 * Esta es la API para el brocker de INVINOS con el que hay que interactuar para realizar transacciones en la blockchain.
 *
 * API version: 1.0
 * Contact: francisco.delicado@uclm.es
 * Generated by: Swagger Codegen (https://github.com/swagger-api/swagger-codegen.git)
 */
package models

// Información química del mosto
type InfoMostoAddQuimica struct {
	// Graduación alcohólica potencial %vol
	Gap float64 `json:"gap,omitempty"`
	// Densidad del vino g/ml
	Densidad float64 `json:"densidad,omitempty"`
	// Otros datos químicos del vino
	Otros string `json:"otros,omitempty"`
}
